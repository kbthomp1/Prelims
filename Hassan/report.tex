...
\begin{enumerate}
  ...
  \item The Pohlhausen method is typically defined as a velocity profile polynomial
    ...
    wedge flows, where $U_e = U_1 x^m$.  In the literature it is reported that
  ...
  \item As stated previously, gibbs free energy is defined as
    \begin{equation}
      ...
      \label{gibbs-formal}
    \end{equation}
    Since $G$ is an extensive property
    \begin{equation}
      G(T,p,\lambda N_1, \lambda N_2,\dots, N_{ns}) 
      = \lambda G(T,p,N_1,N_2,\dots,N_{ns}) 
      \label{extensive-prop}
    \end{equation}
    Thus, by differentiating with respect to the arbitrary constant $\lambda$
    \begin{equation}
      \begin{aligned}
        \pd{G}{\lambda} = G &= \pd{G}{N_1}N_1 + \pd{G}{N_2} + \dots +
        \pd{G}{N_{ns}}N_{ns} \\ G &= \sum\limits_{s=1}^{N_{ns}}{\mu_s N_s}
      \end{aligned}
      \label{gibbs-alt-def}
    \end{equation}
    where $\mu_s$ is the previously defined chemical potential of species $s$.
